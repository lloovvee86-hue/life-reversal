    ...
    for spot in hotspots:
        ...
        if area_code:
            ...
            try:
                ...
                print(f"⚠️ [{spot['areaName']}] API 연동 실패/미오픈 상태. 캐시 데이터를 유지합니다.")
...
"""
    ...
    for spot in hotspots:
        ...
        spots_html = []
        for item in spot['spots']:
            clean_name = item['name'].replace("'", "\\'")
            clean_address = item['address'].replace("'", "\\'")
            spots_html.append(f"""
                    <div class="spot-card" onclick="openMapModal('{clean_name}', '{clean_address}', {item['lat']}, {item['lng']})">
                        <div class="spot-name">
    ...
    <script>
        ...
        function openMapModal(name, address, lat, lng) {{
            ...
            document.getElementById('link-google').href = 'https://www.google.com/maps/search/?api=1&query=' + encodeURIComponent(name + ' ' + address);
            document.getElementById('link-naver').href = 'https://map.naver.com/p/search/' + encodeURIComponent(name);
            document.getElementById('link-kakao').href = 'https://map.kakao.com/?q=' + encodeURIComponent(name);
            document.getElementById('link-tmap').href = 'tmap://route?rGoName=' + encodeURIComponent(name) + '&rGoX=' + lng + '&rGoY=' + lat;
