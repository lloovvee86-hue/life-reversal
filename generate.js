...
async function main() {
  ...
  // 2.5 지도 경로 데이터 로드
  const pathsFile = path.join(DATA_DIR, 'south-korea-paths.json');
  if (!fs.existsSync(pathsFile)) {
    console.error('❌ 지도 경로 데이터가 없습니다.');
    return;
  }
  const pathsData = JSON.parse(fs.readFileSync(pathsFile, 'utf-8'));
  let svgPathsHtml = '';
  for (const [pathId, pathInfo] of Object.entries(pathsData)) {
    svgPathsHtml += `            <path id="${pathId}" aria-label="${pathInfo.name}" d="${pathInfo.d}" class="map-bg-path" />\n`;
  }
...
`;
  ...
  fs.writeFileSync(path.join(PUBLIC_DIR, 'index.html'), htmlContent.replace("<!-- SVG_PATHS_PLACEHOLDER -->", svgPathsHtml), 'utf-8');
